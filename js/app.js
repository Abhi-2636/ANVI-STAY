...
// Utility to fix broken Unsplash URLs
const fixImageUrl = (url) => {
  if (!url) return "https://images.unsplash.com/photo-1598928506311-c55ded91a20c?q=80&w=800";
...
try {
  ...
  if (firebaseConfig && Object.keys(firebaseConfig).length > 0) {
    ...
    // Suppress warning log and show premium development notice
    console.log(
      "%c[Anvi Stay] Initialized in High-Performance Local Mode ✨",
      "color: #C8A24A; font-weight: bold; background: #1B1B1B; padding: 2px 5px; border-radius: 3px;"
    );
  }
} catch (e) {
...
const buildings = [
  ...
  },
  {
    ...
    rent: "7000",
    image:
      "https://images.unsplash.com/photo-1517646280695-4632aa624003?q=80&w=800",
    type: "Semi-Furnished",
...
// Global click delegation for haptics and routing
document.addEventListener("click", (e) => {
  ...
  // Handle cross-view anchor links
  if (link && link.getAttribute("href")?.startsWith("#")) {
    const targetId = link.getAttribute("href").substring(1);
    const validSections = ["services", "properties", "contact", "why-choose", "radar"];
    if (validSections.includes(targetId)) {
      e.preventDefault();
      // Ensure we switch back to the landing view if we're on a portal
      if (typeof state !== "undefined" && state.view !== "landing") {
        if (typeof window.switchView === "function") {
          window.switchView("landing");
        }
      }
      // Wait for layout rendering, then scroll
      setTimeout(() => {
...
async function run() {
  ...
  // Initialize Landing UX (Spatial effects, Staggered entries, Parallax)
  // We call it here directly because app.js might be loaded after DOMContentLoaded
  if (typeof initLandingUX === "function") {
    initLandingUX();
  }
...
function initRouting() {
  const savedAdminToken = localStorage.getItem("anvi_admin_token");
  const savedTenantLogin = localStorage.getItem("anvi_tenant_login");
  
  // Extract route from hash OR pathname (supporting Vercel rewrites)
  ...
  // 1. If we have a saved admin token and no specific non-admin route, go to landlord
  if (savedAdminToken && (!route || route === "landlord" || route === "admin-login" || route === "landing")) {
    switchView("landlord");
    return;
  } 
  ...
  // 3. Handle explicit routes
  if (
    route &&
    route !== "landing" &&
    ["tenant", "admin-login", "landlord", "about", "terms", "privacy", "tos"].includes(route)
  ) {
    if (route === "landlord" && !savedAdminToken) {
...
window.openPropertyModal = (buildingId) => {
  ...
  // Map Integration
  const mapIframe = byId("pm-map");
  if (mapIframe) {
    const mapQuery = encodeURIComponent(`${p.name}, Law Gate, Phagwara, Punjab`);
    mapIframe.src = `https://maps.google.com/maps?q=${mapQuery}&t=&z=17&ie=UTF8&iwloc=&output=embed`;
  }
...
function renderLandingUI() {
  ...
  container.innerHTML = buildings
    .map((p, idx) => {
      ...
      return `
        <div class="group relative bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-[0_8px_30px_-12px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_40px_-12px_rgba(16,185,129,0.15)] hover:border-emerald-200 transition-all duration-500 hover:-translate-y-2 flex flex-col spatial-card stagger-enter ${isFull ? "opacity-80" : ""}" onclick="openRoomDetail(${idx})" style="cursor: pointer;">
          
          <!-- Image Section -->
          <div class="h-56 sm:h-64 relative overflow-hidden">
            <img src="${fixImageUrl(p.image)}" alt="${p.name}" class="w-full h-full object-cover transition-transform duration-700 ease-in-out group-hover:scale-110" loading="lazy" width="800" height="500">
  ...
  content.innerHTML = `
        ...
        <div class="p-6 sm:p-8">
          ...
          <details class="group mb-5">
            <summary class="text-[10px] font-black uppercase tracking-widest flex items-center justify-between cursor-pointer list-none select-none p-3 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors" style="color:#1F3D2B">
              <div class="flex items-center gap-2">
                <span class="w-5 h-5 bg-amber-50 rounded-md flex items-center justify-center"><i class="fas fa-star text-[8px] text-[#C8A24A]"></i></span>
                Amenities Included
              </div>
              <i class="fas fa-chevron-down transition-transform duration-300 group-open:-rotate-180 text-slate-400"></i>
            </summary>
            ...
            <!-- Custom style to ensure default summary arrow is hidden in all browsers -->
            <style>
...
if (window.location.hostname.startsWith('192.168.') || window.location.hostname.startsWith('10.')) {
  // Mobile device testing localhost via local network IP
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < MAX_RETRIES) {
          if (errEl) {
            errEl.textContent = `⚡ Reconnecting... (${attempt}/${MAX_RETRIES})`;
            errEl.classList.remove("hidden");
          }
...
window.switchTenantTab = (tab) => {
  ...
  if (tab === "maintenance") {
    ...
    if (payPage) payPage.classList.remove("hidden");
    window.scrollTo({ top: 0, behavior: "smooth" });
  } else if (tab === "profile") {
...
function showTenantError(msg) {
  ...
  // Only auto-hide non-critical errors (not server wake-up messages)
  if (!msg.includes('⏳') && !msg.includes('waking')) {
...
window.fetchTenantDashboard = async () => {
  ...
  // Disable the login button and show a loading state while fetching
  const loginBtn = byId("tenant-login-btn");
  if (loginBtn) {
    loginBtn.disabled = true;
    loginBtn._origHTML = loginBtn.innerHTML;
    loginBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> Connecting...`;
  }

  try {
    // Faster connection logic for Vercel
    let res;
    const MAX_RETRIES = 2;
    const RETRY_DELAY = 1000; 
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        res = await fetch(`${API_BASE}/rooms/tenant-login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            buildingId: bid,
        ...
        if (attempt < MAX_RETRIES) {
          showTenantError(`⚡ Connecting... (${attempt}/${MAX_RETRIES})`);
          if (loginBtn) loginBtn.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i> Connecting (${attempt}/${MAX_RETRIES})...`;
          await new Promise(r => setTimeout(r, RETRY_DELAY));
        } else {
          throw fetchErr;
        }
    ...
    // Safety check: if res is somehow undefined, throw to catch block
    if (!res) {
    ...
    if (!res.ok) {
      ...
      showTenantError(errData.message || "Access denied. Check credentials.");
      // Restore login button
      if (loginBtn) { loginBtn.disabled = false; loginBtn.innerHTML = loginBtn._origHTML; }
      return;
    }
    ...
    const key = `${bid}-${rno}`;
    state.tenants[key] = t;
    // Store credentials for persistence and complaint submission
    state.tenantLogin = { bid, rno, pass };
    localStorage.setItem("anvi_tenant_login", JSON.stringify(state.tenantLogin));
    ...
    dash.innerHTML = `
        ...
        <div class="welcome-card-ref bg-gradient-to-r ${greetGradient} rounded-[2rem] sm:rounded-[2.5rem] p-8 sm:p-12 text-white mb-6 sm:mb-10 relative overflow-hidden shadow-[0_20px_50px_rgba(16,185,129,0.2)] tilt-card">
            ...
            <div class="flex items-center justify-between mb-8">
              <div>
                <div class="flex items-center gap-2 mb-3 flex-wrap">
                  <span class="px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-widest text-emerald-300 border border-white/10">${(buildings.find((b) => b.id === bid) || { name: bid }).name}</span>
    ...
    if (payPage) {
      payPage.innerHTML = `
        ...
        return `
          <details class="group bg-white/80 backdrop-blur-2xl rounded-[2rem] border border-white p-6 sm:p-10 mb-6 sm:mb-10 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] transition-all">
            <summary class="flex items-center justify-between cursor-pointer list-none select-none outline-none">
              <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-indigo-50 border border-indigo-100/50 rounded-2xl flex items-center justify-center shadow-inner text-indigo-500 transition-colors group-hover:bg-indigo-100">
              ...
              <div class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-100/50 group-hover:bg-slate-100 transition-colors">
                <i class="fas fa-chevron-down text-slate-400 transition-transform duration-300 group-open:-rotate-180"></i>
              </div>
            </summary>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-8 animate-[fadeInUp_0.3s_ease]">
              ${enabledAmenities
            .map(
              (a) => `
    ...
    showTenantError(userMsg);
    toast('Connection error. Please try again in a moment.', 5000);
    // Restore Login button
    const loginBtn = byId("tenant-login-btn");
    if (loginBtn) { loginBtn.disabled = false; if (loginBtn._origHTML) loginBtn.innerHTML = loginBtn._origHTML; }
    // Restore Login UI if failed
    if (loginForm) {
      loginForm.classList.remove("hidden");
      dash.classList.add("hidden");
      dash.classList.remove("dashboard-enter");
    ...
    <div id="upi-payment-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background: rgba(0,0,0,0.6); backdrop-filter: blur(8px);">
      <div class="bg-white rounded-[2rem] shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto animate-slide-up" style="animation: slideUp 0.35s ease-out;">
        ...
        <div class="p-6 space-y-5">
          ...
          <!-- Submit Button -->
          <button id="upi-submit-btn" onclick="submitUpiPayment('${paymentType}', ${amount}, 'upi')"
          ...
          <div class="flex items-center gap-3">
            <div class="flex-1 h-px bg-slate-200"></div>
            <span class="text-[10px] font-black text-slate-300 uppercase tracking-widest">or pay in cash</span>
            <div class="flex-1 h-px bg-slate-200"></div>
          </div>

          <!-- Cash Payment Request Button -->
          <button id="cash-submit-btn" onclick="submitUpiPayment('${paymentType}', ${amount}, 'cash')"
            class="w-full bg-slate-800 hover:bg-slate-900 text-white py-4 rounded-2xl font-bold text-sm shadow-xl hover:shadow-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-3">
            <i class="fas fa-money-bill-wave"></i>
            <span>Request Cash Payment Verification</span>
          </button>
...
// ── Submit Payment (Tenant: UPI or Cash) ──
window.submitUpiPayment = async (paymentType, amount, method = 'upi') => {
  const isCash = method === 'cash';
  const btnId = isCash ? "cash-submit-btn" : "upi-submit-btn";
  const utr = byId("upi-utr-input")?.value?.trim();
  const errEl = byId("upi-submit-error");
  const btn = byId(btnId);

  if (!isCash && (!utr || utr.length < 6)) {
    if (errEl) {
  ...
  try {
    ...
    if (result.success) {
      ...
      toast(
        isCash 
          ? "✅ Cash verification requested! Please visit office with payment."
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = isCash 
          ? '<i class="fas fa-money-bill-wave"></i> <span>Request Cash Payment Verification</span>'
...
window.renderPendingUpiPayments = async () => {
  ...
  try {
    ...
    if (pending.length > 0) {
      ...
      pending.forEach((p) => {
        const isCash = p.method === "cash" || p.utrNumber === "CASH";
        const bName =
              ...
              <div class="flex items-start gap-4">
                ...
                <div>
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-[9px] font-black uppercase tracking-widest ${isCash ? "bg-emerald-100 text-emerald-600" : "bg-indigo-100 text-indigo-600"} px-2 py-0.5 rounded-full">${isCash ? "CASH Verification" : "UPI Transfer"}</span>
                  ...
                  <div class="flex items-center gap-3 mt-2 text-xs">
                    <span class="font-bold text-slate-600"><i class="fas ${isCash ? "fa-money-bill-wave" : "fa-receipt"} text-slate-400 mr-1"></i> ${isCash ? "" : "UTR:"} <span class="text-slate-800 select-all">${isCash ? "Requested via Cash" : p.utrNumber}</span></span>
...
window.saveBillingSpreadsheet = async () => {
  ...
  const promises = [];
  const token = localStorage.getItem("anvi_admin_token");
...
// ══════════════════════════════════════
// ═══ MAINTENANCE BOARD (API-Connected) ════
// ══════════════════════════════════════

// State for maintenance tickets
let _maintTickets = [];
let _maintLoading = false;

// ── Load all tickets from backend ──
window.loadMaintenanceTickets = async (filters = {}) => {
  if (_maintLoading) return;
  _maintLoading = true;
  const token = localStorage.getItem("anvi_admin_token");
  if (!token) { _maintLoading = false; return; }

  // Show loading skeletons
  ["kanban-todo", "kanban-progress", "kanban-done"].forEach(id => {
    const el = byId(id);
    if (el) el.innerHTML = `<div class="space-y-2">${[1,2].map(()=>`<div class="kanban-card animate-pulse"><div class="h-3 bg-slate-100 rounded w-2/3 mb-2"></div><div class="h-2 bg-slate-50 rounded w-full mb-1"></div><div class="h-2 bg-slate-50 rounded w-4/5"></div></div>`).join("")}</div>`;
  });

  try {
    // Build query params
    const params = new URLSearchParams({ limit: 200 });
    const filterEl = byId("kanban-building-filter");
    const catEl = byId("kanban-category-filter");
    if (filterEl?.value && filterEl.value !== "all") params.set("buildingId", filterEl.value);
    if (catEl?.value && catEl.value !== "all") params.set("category", catEl.value);
    if (filters.status) params.set("status", filters.status);

    const [ticketsRes, statsRes] = await Promise.all([
      fetch(`${API_BASE}/maintenance?${params}`, { headers: { Authorization: `Bearer ${token}` } }),
    ...
    if (ticketsData.success) {
      _maintTickets = ticketsData.data || [];
    }

    // Update stat cards
    if (statsData.success && statsData.data?.byStatus) {
      const byStatus = {};
      statsData.data.byStatus.forEach(s => byStatus[s._id] = s.count);
      safeSet("maint-stat-open", "textContent", byStatus["open"] || 0);
      safeSet("maint-stat-progress", "textContent", byStatus["in-progress"] || 0);
      safeSet("maint-stat-resolved", "textContent", byStatus["resolved"] || 0);
      // Urgent count from byPriority
      const byPriority = {};
      (statsData.data.byPriority || []).forEach(p => byPriority[p._id] = p.count);
      safeSet("maint-stat-urgent", "textContent", byPriority["urgent"] || 0);
    }

    renderMaintenanceKanban();
  } catch (err) {
    console.error("[loadMaintenanceTickets]", err);
    toast("Failed to load maintenance tickets", 3000);
  } finally {
    _maintLoading = false;
  }
};
...
window.renderMaintenanceKanban = () => {
  const filterBid = byId("kanban-building-filter")?.value || "all";
  const filterCat = byId("kanban-category-filter")?.value || "all";

  let tickets = _maintTickets;
  if (filterBid !== "all") tickets = tickets.filter(t => t.buildingId === filterBid);
  if (filterCat !== "all") tickets = tickets.filter(t => t.category === filterCat);

  const todo     = tickets.filter(t => t.status === "open");
  const progress = tickets.filter(t => t.status === "in-progress" || t.status === "on-hold");
  const done     = tickets.filter(t => t.status === "resolved" || t.status === "closed");

  // Also pull open complaints from state.tenants (they come from room records)
  const complaintToDo = [];
  Object.entries(state.tenants).forEach(([key, t]) => {
    const [bid, rno] = key.split("-");
    ...
    (t.complaints || []).forEach(c => {
      ...
      complaintToDo.push({ _id: c.id, description: c.text, buildingId: bid, roomNo: rno,
        buildingName: buildName, category: "complaint", priority: "medium",
        status: "open", isComplaint: true, createdAt: c.createdAt });
    });
  });

  const priorityGrad = {
    urgent: "linear-gradient(135deg,#fee2e2,#fff)",
    high:   "linear-gradient(135deg,#fff7ed,#fff)",
    medium: "linear-gradient(135deg,#fffbeb,#fff)",
    low:    "linear-gradient(135deg,#f0fdf4,#fff)",
  };
  const priorityBadge = {
  ...
  const categoryIcon = {
    plumbing: "fa-faucet-drip", electrical: "fa-bolt", furniture: "fa-couch",
    cleaning: "fa-broom", internet: "fa-wifi", security: "fa-shield-halved",
    complaint: "fa-comment-dots", other: "fa-wrench",
  };

  const renderCard = (t, colId) => {
    const isComplaint = t.isComplaint;
    const icon = categoryIcon[t.category] || "fa-wrench";
    const badge = priorityBadge[t.priority] || priorityBadge.medium;
    const grad = priorityGrad[t.priority] || priorityGrad.medium;
    const buildLabel = t.buildingName || t.buildingId;
    const age = t.createdAt ? Math.floor((Date.now() - new Date(t.createdAt)) / 86400000) : 0;

    return `
      <div class="kanban-card cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-lg rounded-2xl p-4 border border-white/60 relative overflow-hidden group/mcard" style="background:${grad}" onclick="openMaintDetail('${t._id}','${colId}')">
        <div class="absolute inset-0 bg-white/40 opacity-0 group-hover/mcard:opacity-100 transition-opacity pointer-events-none"></div>
        <div class="flex items-start justify-between gap-2 mb-3 relative z-10">
          <div class="flex items-center gap-2">
            <div class="w-7 h-7 rounded-xl bg-white shadow-sm border border-slate-100/50 flex items-center justify-center flex-shrink-0 group-hover/mcard:scale-110 transition-transform">
              <i class="fas ${icon} text-[11px] text-slate-500 drop-shadow-sm"></i>
            </div>
            <span class="text-[11px] font-bold text-slate-700 font-mono tracking-tight">${buildLabel} <span class="text-slate-300 mx-0.5">•</span> Rm ${t.roomNo}</span>
          </div>
          <span class="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-md shadow-sm ${badge} border border-white/50">${t.priority}</span>
        </div>
        <div class="text-[13px] font-bold text-slate-800 leading-snug relative z-10">${t.title || t.description}</div>
        ${t.description && t.title ? `<div class="text-[10px] text-slate-500 mt-1 line-clamp-2 relative z-10">${t.description}</div>` : ""}
        
        <div class="flex items-center justify-between mt-4 pt-3 border-t border-slate-200/50 relative z-10">
          <div class="flex items-center gap-1.5">
            ${t.assignedTo ? `<span class="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded"><i class="fas fa-user-astronaut mr-1 text-[9px]"></i>${t.assignedTo}</span>` : `<span class="text-[10px] font-semibold text-slate-400 border border-dashed border-slate-300 px-1.5 py-0.5 rounded">Unassigned</span>`}
          </div>
          <span class="text-[9px] font-black uppercase tracking-widest text-slate-400"><i class="far fa-clock mr-0.5 text-[8px]"></i>${age === 0 ? "Today" : age === 1 ? "Yesterday" : age + "d ago"}</span>
        </div>
        
        <div class="flex items-center gap-2 mt-3 relative z-10">
          ${colId === "kanban-todo" ? `
            <button class="flex-1 py-1.5 rounded-lg text-[10px] font-bold shadow-sm hover:shadow transition-all active:scale-95" style="background:#eff6ff;color:#3b82f6;border:1px solid #bfdbfe;" onclick="event.stopPropagation();updateMaintStatus('${t._id}','in-progress')"><i class="fas fa-play text-[9px] mr-1"></i> Start</button>
            ${isComplaint ? `<button class="flex-1 py-1.5 rounded-lg text-[10px] font-bold shadow-sm hover:shadow transition-all active:scale-95" style="background:#fef3c7;color:#d97706;border:1px solid #fde68a;" onclick="event.stopPropagation();escalateComplaint('${t._id}')"><i class="fas fa-arrow-up text-[9px] mr-1"></i> Escalate</button>` : ""}
          ` : ""}
          ${colId === "kanban-progress" ? `
            <button class="flex-1 py-1.5 rounded-lg text-[10px] font-bold shadow-sm hover:shadow transition-all active:scale-95" style="background:#f0fdf4;color:#22c55e;border:1px solid #bbf7d0;" onclick="event.stopPropagation();updateMaintStatus('${t._id}','resolved')"><i class="fas fa-check-double text-[9px] mr-1"></i> Resolve</button>
            <button class="px-2 py-1.5 rounded-lg text-[10px] font-bold shadow-sm hover:shadow transition-all active:scale-95" style="background:#f1f5f9;color:#64748b;border:1px solid #e2e8f0;" onclick="event.stopPropagation();updateMaintStatus('${t._id}','on-hold')"><i class="fas fa-pause text-[9px]"></i></button>
          ` : ""}
          ${colId === "kanban-done" ? `<div class="w-full flex justify-end"><span class="text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 flex items-center gap-1"><i class="fas ${t.status === "closed" ? "fa-lock" : "fa-check-double"}"></i> ${t.status === "closed" ? "Closed" : "Resolved"}</span></div>` : ""}
        </div>
      </div>`;
  };

  const setColumn = (arr, id, label) => {
    const el = byId(id);
    if (!el) return;
    const total = arr.length + (id === "kanban-todo" ? complaintToDo.length : 0);
    if (total === 0) {
      el.innerHTML = `<div class="text-center py-8"><i class="fas fa-clipboard-check text-3xl text-slate-100 mb-2 block"></i><p class="text-slate-300 text-xs">No ${label.toLowerCase()} tickets</p></div>`;
      return;
    }
    const allCards = id === "kanban-todo" ? [...arr, ...complaintToDo] : arr;
    el.innerHTML = allCards.map(c => renderCard(c, id)).join("");
  };

  setColumn(todo,     "kanban-todo",     "open");
  setColumn(progress, "kanban-progress", "in-progress");
  setColumn(done,     "kanban-done",     "resolved");
  safeSet("kanban-count-todo",     "textContent", todo.length + complaintToDo.length);
  safeSet("kanban-count-progress", "textContent", progress.length);
  safeSet("kanban-count-done",     "textContent", done.length);
};

// ── Update ticket status via API ──
window.updateMaintStatus = async (id, status) => {
  const token = localStorage.getItem("anvi_admin_token");
  if (!token) return;
  try {
    ...
    const data = await res.json();
    if (data.success) {
      toast(status === "resolved" ? "✓ Ticket resolved!" : status === "in-progress" ? "→ Ticket started" : "Ticket updated", 2000);
      await loadMaintenanceTickets();
    } else {
      toast("Failed to update ticket", 3000);
    }
  } catch (err) {
    console.error("[updateMaintStatus]", err);
    toast("Network error updating ticket", 3000);
  }
};

// ── Open ticket detail / edit modal ──
window.openMaintDetail = (id, colId) => {
  const t = _maintTickets.find(x => x._id === id);
  if (!t) return;

  const categoryIcon2 = { plumbing:"🚰", electrical:"⚡", furniture:"🪑", cleaning:"🧹", internet:"📶", security:"🔒", other:"🔧" };
  const statusColor = { open:"text-amber-600 bg-amber-50", "in-progress":"text-blue-600 bg-blue-50", "on-hold":"text-slate-600 bg-slate-50", resolved:"text-emerald-600 bg-emerald-50", closed:"text-slate-400 bg-slate-50" };
  const priorityColor = { urgent:"text-rose-700 bg-rose-50", high:"text-orange-700 bg-orange-50", medium:"text-amber-700 bg-amber-50", low:"text-emerald-700 bg-emerald-50" };
  const age = t.createdAt ? Math.floor((Date.now() - new Date(t.createdAt)) / 86400000) : 0;
  const createdStr = t.createdAt ? new Date(t.createdAt).toLocaleString("en-IN", { day:"numeric", month:"short", year:"numeric", hour:"2-digit", minute:"2-digit" }) : "";

  const overlay = document.createElement("div");
  overlay.id = "maint-detail-overlay";
  overlay.className = "fixed inset-0 z-[99999] flex items-center justify-center p-4";
  overlay.style.cssText = "background:rgba(0,0,0,0.5);backdrop-filter:blur(8px);animation:viewFadeIn 0.2s ease";
  overlay.innerHTML = `
    <div class="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl" style="animation:viewFadeIn 0.3s ease">
      <div class="flex items-center justify-between p-6 border-b border-slate-100">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-slate-100 rounded-2xl flex items-center justify-center text-lg">${categoryIcon2[t.category] || "🔧"}</div>
          <div>
            <h3 class="text-sm font-black text-slate-800">${t.title || t.description.slice(0,40)}</h3>
            <p class="text-[10px] text-slate-400 font-bold">Ticket #${t._id.slice(-6).toUpperCase()}</p>
          </div>
        </div>
        <button onclick="document.getElementById('maint-detail-overlay').remove()" class="w-8 h-8 bg-slate-100 hover:bg-rose-100 rounded-xl flex items-center justify-center text-slate-400 hover:text-rose-500 transition">
      ...
      <div class="p-6 space-y-4">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <span class="text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${statusColor[t.status] || "text-slate-600 bg-slate-50"}">${t.status.replace("-"," ")}</span>
          <span class="text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full ${priorityColor[t.priority] || "text-slate-600 bg-slate-50"}">${t.priority} priority</span>
          <span class="text-[10px] font-semibold text-slate-400">${age === 0 ? "Opened today" : age + " days open"}</span>
        </div>
        <!-- Details grid -->
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-slate-50 rounded-xl p-3">
            <p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1">Location</p>
            <p class="text-xs font-bold text-slate-700">${t.buildingId} · Room ${t.roomNo}</p>
          </div>
          <div class="bg-slate-50 rounded-xl p-3">
            <p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1">Category</p>
            <p class="text-xs font-bold text-slate-700">${t.category}</p>
          </div>
          <div class="bg-slate-50 rounded-xl p-3">
            <p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1">Assigned To</p>
            <p class="text-xs font-bold text-slate-700">${t.assignedTo || "Unassigned"}</p>
          </div>
          <div class="bg-slate-50 rounded-xl p-3">
            <p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-1">Reported By</p>
            <p class="text-xs font-bold text-slate-700 capitalize">${t.reportedBy || "admin"}</p>
          </div>
          ${t.estimatedCost ? `<div class="bg-amber-50 rounded-xl p-3"><p class="text-[9px] font-black uppercase tracking-wider text-amber-500 mb-1">Est. Cost</p><p class="text-xs font-bold text-amber-700">₹${t.estimatedCost.toLocaleString()}</p></div>` : ""}
          ${t.actualCost ? `<div class="bg-emerald-50 rounded-xl p-3"><p class="text-[9px] font-black uppercase tracking-wider text-emerald-500 mb-1">Actual Cost</p><p class="text-xs font-bold text-emerald-700">₹${t.actualCost.toLocaleString()}</p></div>` : ""}
        </div>
        <!-- Description -->
        <div class="bg-slate-50 rounded-xl p-4">
          <p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-2">Description</p>
          <p class="text-sm text-slate-700 leading-relaxed">${t.description}</p>
        </div>
        ${t.resolutionNote ? `<div class="bg-emerald-50 rounded-xl p-4"><p class="text-[9px] font-black uppercase tracking-wider text-emerald-500 mb-2">Resolution Note</p><p class="text-sm text-emerald-700 leading-relaxed">${t.resolutionNote}</p></div>` : ""}
        ${t.photoUrl ? `<div><p class="text-[9px] font-black uppercase tracking-wider text-slate-400 mb-2">Photo</p><img src="${t.photoUrl}" class="w-full rounded-xl object-cover max-h-40" onerror="this.style.display='none'"/></div>` : ""}
        <!-- Quick update fields -->
        <div class="border-t border-slate-100 pt-4 space-y-3">
          <p class="text-[9px] font-black uppercase tracking-wider text-slate-400">Quick Update</p>
          <div class="grid grid-cols-2 gap-2">
            <select id="maint-edit-status" class="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold" onchange="">
              ${["open","in-progress","on-hold","resolved","closed"].map(s=>`<option value="${s}" ${t.status===s?"selected":""}>${s.replace("-"," ")}</option>`).join("")}
            </select>
            <select id="maint-edit-priority" class="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold">
              ${["low","medium","high","urgent"].map(p=>`<option value="${p}" ${t.priority===p?"selected":""}>${p}</option>`).join("")}
            </select>
          </div>
          <input id="maint-edit-assigned" type="text" placeholder="Assign to staff name..." value="${t.assignedTo || ""}" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold" />
          <input id="maint-edit-cost" type="number" placeholder="Actual cost (₹)" value="${t.actualCost || ""}" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold" />
          <textarea id="maint-edit-note" placeholder="Resolution note..." rows="2" class="w-full px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold resize-none">${t.resolutionNote || ""}</textarea>
        </div>
        <!-- Action buttons -->
        <div class="flex gap-2">
          <button onclick="saveMaintEdit('${t._id}')" class="flex-1 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-black transition">
            <i class="fas fa-save mr-1"></i> Save Changes
          </button>
          <button onclick="deleteMaintTicket('${t._id}')" class="w-10 h-10 bg-rose-50 hover:bg-rose-100 text-rose-500 rounded-xl flex items-center justify-center transition flex-shrink-0">
            <i class="fas fa-trash-alt text-xs"></i>
          </button>
        </div>
        <p class="text-[10px] text-slate-300 text-center">Created: ${createdStr}</p>
      </div>
    </div>`;
...
window.saveMaintEdit = async (id) => {
  const token = localStorage.getItem("anvi_admin_token");
  if (!token) return;
  const updates = {
    status: byId("maint-edit-status")?.value,
    priority: byId("maint-edit-priority")?.value,
    assignedTo: byId("maint-edit-assigned")?.value || "",
    actualCost: Number(byId("maint-edit-cost")?.value) || 0,
    resolutionNote: byId("maint-edit-note")?.value || "",
  };
  try {
    const res = await fetch(`${API_BASE}/maintenance/${id}`, {
      method: "PUT",
    ...
    const data = await res.json();
    if (data.success) {
      toast("✓ Ticket updated!", 2000);
      byId("maint-detail-overlay")?.remove();
      await loadMaintenanceTickets();
    } else toast("Failed to save", 3000);
  } catch { toast("Network error", 3000); }
};

window.deleteMaintTicket = async (id) => {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) {
      toast("Ticket deleted", 2000);
      byId("maint-detail-overlay")?.remove();
      await loadMaintenanceTickets();
    } else toast("Failed to delete", 3000);
  } catch { toast("Network error", 3000); }
};

// ── Add new ticket via form ──
window.addKanbanTicket = async () => {
  const text = byId("kanban-ticket-input")?.value?.trim();
  if (!text) { toast("Enter a description", 2000); return; }
  const bid = byId("kanban-ticket-building")?.value || buildings[0]?.id;
  const rno = byId("kanban-ticket-room")?.value;
  if (!rno) { toast("Select a room", 2000); return; }

  const token = localStorage.getItem("anvi_admin_token");
  if (!token) return;

  const btn = byId("kanban-add-btn");
  ...
  try {
    const res = await fetch(`${API_BASE}/maintenance`, {
      ...
      body: JSON.stringify({
        ...
        priority: byId("kanban-ticket-priority")?.value || "medium",
        category: byId("kanban-ticket-category")?.value || "other",
        assignedTo: byId("kanban-ticket-assigned")?.value || "",
        reportedBy: "admin",
      }),
    });
    const data = await res.json();
    if (data.success) {
      if (byId("kanban-ticket-input")) byId("kanban-ticket-input").value = "";
      toast("✓ Maintenance ticket created!", 2500);
      await loadMaintenanceTickets();
    } else toast("Failed to create ticket", 3000);
  } catch { toast("Network error", 3000); }
  finally {
    if (btn) { btn.disabled = false; btn.innerHTML = `<i class="fas fa-plus mr-1"></i> Add`; }
  }
};

// ── Legacy alias for compatibility ──
window.renderKanbanBoard = () => { loadMaintenanceTickets(); };

// ── Escalate complaint (promote from room record to Maintenance ticket) ──
window.escalateComplaint = async (complaintId) => {
  const token = localStorage.getItem("anvi_admin_token");
  if (!token) return;
  // Find the complaint
  let complaint = null, roomKey = null;
  Object.entries(state.tenants).forEach(([key, t]) => {
    const found = (t.complaints || []).find(c => c.id === complaintId);
    if (found) { complaint = found; roomKey = key; }
  });
  if (!complaint || !roomKey) { toast("Complaint not found", 2000); return; }
  const [bid, rno] = roomKey.split("-");
  const t = state.tenants[roomKey];
  try {
    const res = await fetch(`${API_BASE}/maintenance`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        buildingId: bid, roomNo: Number(rno), description: complaint.text,
        title: "Escalated: " + complaint.text.slice(0, 40),
        priority: "high", category: "other",
        tenantName: t?.name || "", reportedBy: "tenant",
      }),
    });
    const data = await res.json();
    if (data.success) {
      toast("✓ Complaint escalated to maintenance ticket", 2500);
      await loadMaintenanceTickets();
    }
  } catch { toast("Network error", 2000); }
};
...
function initListeners() {
  ...
  // Smooth scroll for footer and miscellaneous section links (e.g. #contact, #amenities)
  document.querySelectorAll('a[href^="#"]').forEach((anchor) => {
    anchor.addEventListener("click", function (e) {
      const href = this.getAttribute("href");
      // Skip portal routes and top-level views
      if (["#tenant", "#admin-login", "#landlord", "#landing"].includes(href)) return;
      
      e.preventDefault();
      const target = document.querySelector(href);
      if (target) {
        // If the target is inside a view that is currently hidden, switch to landing first
        if (state.view !== 'landing') {
          navigateTo('landing');
          // Delay scroll until view switch finishes
          setTimeout(() => {
...
// ── Persistent notification dismiss storage ──
const ADMIN_DISMISSED_KEY = "anvi_admin_dismissed_notifs";
const TENANT_DISMISSED_KEY = "anvi_tenant_dismissed_notifs";

function getAdminDismissed() {
  try { return new Set(JSON.parse(localStorage.getItem(ADMIN_DISMISSED_KEY)) || []); } catch { return new Set(); }
}
function saveAdminDismissed(set) {
  localStorage.setItem(ADMIN_DISMISSED_KEY, JSON.stringify([...set]));
}
function getTenantDismissed() {
  try { return new Set(JSON.parse(localStorage.getItem(TENANT_DISMISSED_KEY)) || []); } catch { return new Set(); }
}
function saveTenantDismissed(set) {
  localStorage.setItem(TENANT_DISMISSED_KEY, JSON.stringify([...set]));
}
...
function renderAdminNotifs() {
  ...
  const notifs = (state.adminNotifs || []).filter(n => !dismissed.has(n.id));

  if (badge) {
    badge.textContent = notifs.length || "";
    badge.dataset.count = notifs.length;
    // Pulse animation when count > 0
    badge.style.animation = notifs.length > 0 ? "notifPop 0.3s ease" : "none";
  }

  if (!notifs.length) {
    list.innerHTML = '<div class="px-5 py-8 text-center"><i class="fas fa-bell-slash text-slate-200 text-3xl mb-3 block"></i><p class="text-slate-400 text-sm font-medium">All caught up!</p><p class="text-slate-300 text-xs mt-1">No new notifications</p></div>';
    return;
  }

  const colorMap = {
    rose: { bg: "bg-rose-50", text: "text-rose-500", dot: "bg-rose-500", border: "border-rose-100" },
    amber: { bg: "bg-amber-50", text: "text-amber-500", dot: "bg-amber-500", border: "border-amber-100" },
    yellow: { bg: "bg-yellow-50", text: "text-yellow-500", dot: "bg-yellow-500", border: "border-yellow-100" },
    emerald: { bg: "bg-emerald-50", text: "text-emerald-500", dot: "bg-emerald-500", border: "border-emerald-100" },
    blue: { bg: "bg-blue-50", text: "text-blue-500", dot: "bg-blue-500", border: "border-blue-100" },
    purple: { bg: "bg-purple-50", text: "text-purple-500", dot: "bg-purple-500", border: "border-purple-100" },
  };

  list.innerHTML = notifs.map(n => {
    const c = colorMap[n.color] || colorMap.rose;
    return `
      <div class="notif-item group" id="admin-notif-${n.id}">
        <div class="w-9 h-9 ${c.bg} rounded-xl flex items-center justify-center flex-shrink-0 border ${c.border}">
          <i class="fas ${n.icon || "fa-bell"} ${c.text} text-xs"></i>
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-xs font-black text-slate-800 leading-snug">${n.title}</p>
          <p class="text-[11px] text-slate-500 mt-0.5 line-clamp-2">${n.body}</p>
          <p class="text-[10px] text-slate-400 mt-1 flex items-center gap-1"><i class="fas fa-clock text-[8px]"></i> ${n.time}</p>
        </div>
        <button onclick="dismissAdminNotif('${n.id}')" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-rose-100 text-slate-400 hover:text-rose-500 flex items-center justify-center flex-shrink-0 transition opacity-0 group-hover:opacity-100" title="Dismiss">
          <i class="fas fa-xmark text-xs"></i>
        </button>
      </div>`;
  }).join("");
...
window.dismissAdminNotif = (id) => {
  const dismissed = getAdminDismissed();
  dismissed.add(id);
  saveAdminDismissed(dismissed);
  const el = byId(`admin-notif-${id}`);
  if (el) { el.style.transition = "all 0.3s"; el.style.opacity = "0"; el.style.maxHeight = "0"; setTimeout(() => el.remove(), 300); }
  setTimeout(renderAdminNotifs, 350);
};
...
function renderTenantNotifs(notices, complaints) {
  ...
  const dismissed = getTenantDismissed();
  const items = [];

  // Notices as notifications
  (notices || []).forEach((n) => {
    const id = `notice-${n._id || n.text?.slice(0,10)}`;
  ...
  if (badge) {
    ...
    badge.dataset.count = items.length;
  }
  ...
  list.innerHTML = items.map(n => {
    ...
    return `
      <div class="notif-item group" id="tenant-notif-${n.id}">
        <div class="w-9 h-9 ${c.bg} rounded-xl flex items-center justify-center flex-shrink-0 border ${c.border}">
          <i class="fas ${n.icon} ${c.text} text-xs"></i>
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-xs font-black text-slate-800 leading-snug">${n.title}</p>
          <p class="text-[11px] text-slate-500 mt-0.5 line-clamp-2">${n.body}</p>
        ...
        <button onclick="dismissTenantNotif('${n.id}')" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-rose-100 text-slate-400 hover:text-rose-500 flex items-center justify-center flex-shrink-0 transition opacity-0 group-hover:opacity-100" title="Dismiss">
          <i class="fas fa-xmark text-xs"></i>
        </button>
      </div>`;
  }).join("");
...
window.dismissTenantNotif = (id) => {
  const dismissed = getTenantDismissed();
  dismissed.add(id);
  saveTenantDismissed(dismissed);
  const el = byId(`tenant-notif-${id}`);
  if (el) { el.style.transition = "all 0.3s"; el.style.opacity = "0"; el.style.maxHeight = "0"; setTimeout(() => el.remove(), 300); }
  ...
  setTimeout(() => {
    ...
    if (login) {
      const key = `${login.bid}-${login.rno}`;
      const t = state.tenants[key];
      // Refetch notices from cache
      fetch(`${API_BASE}/notices`).then(r=>r.json()).then(d => {
        renderTenantNotifs(d.success ? d.data : [], t?.complaints || []);
      }).catch(() => renderTenantNotifs([], t?.complaints || []));
    }
  }, 350);
};

// ── Clear / Mark-all-read notifications ──
window.clearAdminNotifs = () => {
  // Persist all current notification IDs as dismissed
  const dismissed = getAdminDismissed();
  (state.adminNotifs || []).forEach(n => dismissed.add(n.id));
  saveAdminDismissed(dismissed);
  state.adminNotifs = [];
  renderAdminNotifs();
  toast("✓ All notifications dismissed", 2000);
};
window.clearTenantNotifs = () => {
  // Save all tenant notif IDs as dismissed
  const list = byId("tenant-notif-list");
  if (list) {
    const dismissed = getTenantDismissed();
    list.querySelectorAll("[id^='tenant-notif-']").forEach(el => {
      dismissed.add(el.id.replace("tenant-notif-", ""));
    });
    saveTenantDismissed(dismissed);
  }
  const badge = byId("tenant-notif-badge");
  if (badge) { badge.textContent = ""; badge.dataset.count = "0"; }
  if (list) list.innerHTML = '<div class="px-5 py-8 text-center"><i class="fas fa-bell-slash text-slate-200 text-3xl mb-3 block"></i><p class="text-slate-400 text-sm font-medium">All caught up!</p></div>';
  toast("✓ All notifications dismissed", 2000);
};
...
function scanAdminNotifications() {
  ...
  if (openComplaints.length > 0) {
    ...
    openComplaints.forEach((c) => {
      notifs.push({
        id: `complaint-${c.id || c._id || (c.room + "-" + (c.createdAt || ""))}`,
        title: `Complaint — Room ${c.room.split("-")[1]}`,
        body: c.text,
        icon: "fa-comment-dots",
        color: "rose",
        time: c.createdAt ? new Date(c.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : now,
      });
    });
  }
  state.prevOpenComplaints = openComplaints.length;

  // Month stamp for recurring rent/elec notifs (auto-resets each month)
  const monthStamp = new Date().toISOString().slice(0, 7); // "2026-03"

  // Unpaid rent notifications
  if (unpaidRent > 0) {
    notifs.push({
      id: `unpaid-rent-${monthStamp}`,
  ...
  // Pending UPI payment approvals
  let pendingUpi = [];
  allTenants.forEach(([key, t]) => {
    if (t.status !== "Occupied" && t.status !== "Booked") return;
    (t.pendingPayments || []).forEach(p => {
      if (p.status === "pending") pendingUpi.push({ ...p, room: key });
    });
  });
  if (pendingUpi.length > 0) {
    notifs.push({
      id: `pending-upi-${pendingUpi.map(p=>p.id||p._id).join("-")}`.slice(0, 60),
      title: `${pendingUpi.length} Payment${pendingUpi.length > 1 ? "s" : ""} Awaiting Approval`,
      body: `${pendingUpi.length} UPI screenshot${pendingUpi.length > 1 ? "s need" : " needs"} your review`,
      icon: "fa-credit-card",
      color: "purple",
      time: now,
    });
...
function renderTestimonials() {
  ...
  const cards = testimonialData
    .map(
      (t) => `
        <div class="testimonial-card relative group bg-white p-8 rounded-[2rem] border border-slate-100 hover:-translate-y-2 transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.08)] hover:border-emerald-100">
          <!-- Glass quote icon -->
          <div class="absolute top-4 right-6 text-7xl text-slate-50 font-serif leading-none select-none group-hover:text-emerald-500/10 transition-colors">"</div>
          <div class="relative z-10">
            <div class="flex items-center gap-1 mb-5">
              ${'<i class="fas fa-star text-[#C8A24A] text-xs"></i>'.repeat(t.rating)}
              ${'<i class="far fa-star text-slate-200 text-xs"></i>'.repeat(5 - t.rating)}
            </div>
            <p class="text-[14px] font-medium text-slate-500 leading-relaxed mb-8 italic group-hover:text-slate-700 transition-colors">"${t.text}"</p>
            <div class="flex items-center gap-4 pt-6 border-t border-slate-50">
              <div class="relative w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-900 border border-slate-100 shadow-sm group-hover:bg-emerald-50 transition-colors">
                <span class="font-black text-sm uppercase tracking-wide text-[#C8A24A]">${t.name.charAt(0)}</span>
                <div class="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 border-2 border-white rounded-full flex items-center justify-center shadow-lg" title="Verified Resident">
                  <i class="fas fa-check text-[8px] text-white"></i>
                </div>
              </div>
              <div>
                <p class="text-[14px] font-black tracking-tight text-slate-900 leading-tight">${t.name}</p>
                <p class="text-[10px] font-bold text-[#C8A24A] uppercase tracking-[0.15em] mt-1">${t.course}</p>
              </div>
            </div>
          </div>
        </div>
...
window.openCompareModal = () => {
  ...
  // Remove any existing compare modal
  byId("compare-modal-overlay")?.remove();
  ...
  overlay.innerHTML = `
    <div class="bg-white rounded-3xl p-6 sm:p-8 max-w-4xl w-full max-h-[85vh] overflow-auto shadow-2xl" style="animation:viewFadeIn 0.4s ease">
      <div class="flex justify-between items-center mb-6">
        ...
        <button onclick="closeCompareModal()" class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition"><i class="fas fa-xmark"></i></button>
      </div>
...
window.closeCompareModal = () => {
  const overlay = byId("compare-modal-overlay");
  if (overlay) {
    overlay.style.opacity = "0";
    overlay.style.transition = "opacity 0.25s ease";
    setTimeout(() => overlay.remove(), 250);
  }
  // Clear selection and remove compare bar
  compareList = [];
  updateCompareBar();
};
...
window.addEventListener("beforeinstallprompt", (e) => {
  // Allow the native prompt and the custom banner to coexist for testing
  deferredPrompt = e;
  
  // Show our custom notification too for a premium feel
  if (!localStorage.getItem("anvi_pwa_dismissed")) {
...
window.renderActivityTimeline = () => {
  ...
  fetch(`${API_BASE}/api/audit?limit=20`, {
    headers: { Authorization: `Bearer ${token}` },
  })
    .then((r) => r.json())
    .then((data) => {
...
// ── #15: Full Audit Log Loader (called by Refresh button in Audit Trail tab) ──
window.loadAuditLogs = async () => {
  const listEl = byId("audit-log-list");
  if (!listEl) return;
  ...
  // Show loading
  listEl.innerHTML = `<p class="text-center text-slate-400 text-xs py-8"><i class="fas fa-spinner fa-spin mr-2"></i>Loading activity logs...</p>`;

  try {
    const actionFilter = byId("audit-action-filter")?.value || "";
    const url = `${API_BASE}/audit?limit=100${actionFilter ? "&action=" + actionFilter : ""}`;
    const [logsRes, statsRes] = await Promise.all([
      fetch(url, { headers: { Authorization: `Bearer ${token}` } }),
      fetch(`${API_BASE}/audit/stats`, { headers: { Authorization: `Bearer ${token}` } }),
    ]);
    ...
    if (statsData.success && statsData.data) {
      ...
      const todayEl = byId("audit-stat-today");
      const weekEl  = byId("audit-stat-week");
      const totalEl = byId("audit-stat-total");
    ...
    if (!logsData.success || !logsData.data?.length) {
      listEl.innerHTML = `<p class="text-center text-slate-400 text-xs py-8"><i class="fas fa-clock-rotate-left mr-2"></i>No activity logs found.</p>`;
      return;
    }

    const actionColors = {
      admin_login: "bg-purple-100 text-purple-700",
      tenant_login: "bg-blue-100 text-blue-700",
      room_updated: "bg-amber-100 text-amber-700",
      room_created: "bg-emerald-100 text-emerald-700",
      room_deleted: "bg-rose-100 text-rose-700",
      ...
      file_uploaded: "bg-sky-100 text-sky-700",
    };

    listEl.innerHTML = logsData.data.map(log => {
      ...
      const time = log.timestamp
        ...
        : "";
      const roleIcon = log.performedByRole === "tenant" ? "fa-user" : log.performedByRole === "admin" ? "fa-shield-halved" : "fa-circle";
      return `
        <div class="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition border border-transparent hover:border-slate-100">
          <div class="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0 mt-0.5">
            <i class="fas ${roleIcon} text-slate-500 text-xs"></i>
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <span class="text-xs font-black text-slate-700 truncate">${log.description || log.action || "Action"}</span>
              <span class="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${colorClass}">${log.action || ""}</span>
            </div>
            <p class="text-[10px] text-slate-400 mt-0.5"><span class="font-bold text-slate-500">${log.performedBy || "System"}</span> · ${time}${log.ipAddress ? ` · ${log.ipAddress}` : ""}</p>
          </div>
        </div>`;
    }).join("");
  } catch (err) {
    console.error("[loadAuditLogs]", err);
    listEl.innerHTML = `<p class="text-center text-rose-400 text-xs py-8"><i class="fas fa-triangle-exclamation mr-2"></i>Failed to load logs. Check connection.</p>`;
  }
};
...
window.submitMaintenanceWork = async () => {
  ...
  try {
    const resp = await fetch(
      `${API_BASE}/api/rooms/${buildingId}/${roomNo}/maintenance`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
if (_prevSetAdminTab) {
  window.setAdminTab = (tab) => {
    ...
    if (tab === "audit-trail") {
      setTimeout(renderActivityTimeline, 500);
...
document.addEventListener("submit", (e) => {
  if (e.target.id === "contact-form") {
    ...
    setTimeout(() => {
      toast("Message sent successfully! We will contact you soon. ✅", 5000, "success");
      e.target.reset();
      btn.disabled = false;
      btn.innerHTML = originalContent;
    }, 1500);
...
// ══════════════════════════════════════════════════════════════
// ═══ NEXT-GEN Landing UI/UX Scripts ═══
// ══════════════════════════════════════════════════════════════
function initLandingUX() {
  // 1. Spatial UI Cursor Tracking
  document.addEventListener('mousemove', (e) => {
  ...
  // 2. Staggered Orchestrated Entrances
  if ("IntersectionObserver" in window) {
    const staggerObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry, index) => {
        if (entry.isIntersecting) {
           // Provide staggered delay based on DOM order or observer array index
           setTimeout(() => {
  ...
  window.addEventListener('scroll', () => {
    ...
    document.querySelectorAll('.kinetic-text').forEach((el) => {
      ...
      const direction = el.getAttribute('data-direction') === 'right' ? 1 : -1;
      el.style.transform = `translateY(-50%) translateX(${scrollY * speed * direction}px)`;
    });
  }, { passive: true });
}

// Call Landing UX init immediately as well (for script modules loaded after DOM content)
initLandingUX();
