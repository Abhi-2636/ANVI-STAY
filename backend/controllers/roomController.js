...
exports.submitUpiPayment = async (req, res) => {
  try {
    const { buildingId, roomNo } = req.params;
    const { password, type, amount, utrNumber, screenshotUrl } = req.body;

    if (!password || !type || !utrNumber) {
      return res.status(400).json({ success: false, message: 'Password, payment type, and UTR number are required.' });
    }
